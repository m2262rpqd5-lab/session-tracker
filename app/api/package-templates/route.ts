...
export async function POST(req: NextRequest) {
  ...
  const { name, sessionCount, price, currency, validityDays } = body;
  ...
  const template = await prisma.packageTemplate.create({
    data: { name, sessionCount: Number(sessionCount), price: Number(price), currency: currency ?? "GBP", validityDays: validityDays ? Number(validityDays) : null },
  });
