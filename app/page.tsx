      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          {pendingCalendarEvents > 0 && (
            <Link
              href="/calendar"
              className="flex items-center gap-1.5 text-sm bg-amber-50 text-amber-700 border border-amber-200 px-3 py-1.5 rounded-lg hover:bg-amber-100 transition-colors"
            >
              <AlertCircle size={14} />
              ...
              {clients.map((client: any) => (
                <tr key={client.id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-5 py-4 text-gray-700">
                    ${(client.activePackage?.totalPaid ?? 0).toLocaleString()}
                  </td>
                  <td className="px-5 py-4">
                    {client.activePackage ? (
                      <StatusBadge status={client.activePackage.status} />
                    ) : (
                  ...
                  <td className="px-5 py-4 text-gray-500 text-xs">
                    {client.activePackage?.expiryDate
                      ? format(new Date(client.activePackage.expiryDate), "MMM d, yyyy")
                      : "—"}
                  </td>
                </tr>
              ))}
